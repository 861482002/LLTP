...
class VoxelNet(SingleStage3DDetector):
	...
	def extract_feat_attention(self, points, img_metas=None):
		...
		# num_points: [batch_size * 6542]   #这一批中所有柱子的个数
		# coors: [batch_size * 6542 , 4]

		# voxels, num_points, coors = self.voxelize_transformer(points)  # 一些格子，每个格子中的点的数量，格子在原始场景中的坐标
		voxel_features = self.voxel_encoder(voxels, num_points, coors)
		# 上一步会得到一个 (num_voxel ,feature) 比如 （6430 * batch_size ,64)
		...
		# 	feature_transfomre = My_Transformer_features(feature.size(-1),2,2048,4).to(device=device_gpu)
	...
	def voxelize_transformer(self, points):
		...
		for i, voxel in enumerate(voxels):
			...
			# # 空间注意力编码
			# xyz_transformer = My_Transformer_xyz(voxel_xyz.size(-1), 1, 1024,4).to(device)
			# # 特征注意力编码
			...
			# atten_feature = features_transformer(voxel_feature)
			#
			# atten_voxels.append(torch.cat([atten_xyz, atten_feature], dim=-1))


			'''
			这里是何在一起的一起注意力
			'''
			transformer = My_Transformer_features(8,1,n_encoder=4).to(device)
			atten_voxel = transformer(voxel)
