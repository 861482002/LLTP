...
class DynamicVFE(nn.Module):
    ...
    def forward(self,
        ...
        if self._with_cluster_center:   #计算每一个体素的距离聚类中心的位移      self.cluster_scatter这个函数就是将点都分配到体素中，最后得到很多非空体素，并且每个体素的特征为该体素块中的所有点的最大值
            voxel_mean, mean_coors = self.cluster_scatter(features, coors)  #得到非空体素的特征，每个体素对应于现实空间中的位置坐标
            points_mean = self.map_voxel_center_to_point(
                coors, voxel_mean, mean_coors)  #得到每一个点对应于哪一个体素的特征，体素的特征就是对每个非空体素的特征经过了pointnet操作后的单一体素的全局特征，就相当于一个聚类中心了
            # TODO: maybe also do cluster for reflectivity
            f_cluster = features[:, :3] - points_mean[:, :3]    #这里我们使用每个点减去它对应的体素的特征（也可以称之为聚类中心特征），得到距离聚类的距离也当作一个特征加到点的最后面
        ...
        for i, vfe in enumerate(self.vfe_layers):
            ...
            if i != len(self.vfe_layers) - 1:
                ...
                feat_per_point = self.map_voxel_center_to_point(    #这个函数就是求得每个点与子对应的体素，并且把每个体素的特征拼接到每个点后面形成新的特征
