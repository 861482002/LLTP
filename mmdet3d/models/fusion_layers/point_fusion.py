...
class PointFusion(BaseModule):
    ...
    def forward(self, img_feats, pts, pts_feats, img_metas):
        ...
        """
        img_pts = self.obtain_mlvl_feats(img_feats, pts, img_metas)     #这里面的特征仅仅是经过了resnet50和fpn后的5个不同尺度的特征图的特征，没有原始点云的特征
        img_pre_fuse = self.img_transform(img_pts)                      #经过transform后把640变成128（与点云特征维度相同）然后再相加，（其实我觉得拼接可能会更好一点）
